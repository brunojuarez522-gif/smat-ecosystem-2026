...
def eliminar_estacion(estacion_id: int, db: Session = Depends(database.get_db), user=Depends(auth.validar_token)):
    # Buscar la estación que queremos eliminar
...
def editar_estacion(estacion_id: int, estacion_update: schemas.EstacionCreate, db: Session = Depends(database.get_db), user=Depends(auth.validar_token)):
    # Buscamos la estación que queremos editar
    estacion = db.query(models.EstacionDB).filter(models.EstacionDB.id == estacion_id).first()
